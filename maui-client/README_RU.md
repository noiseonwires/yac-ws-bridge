...
> **См. также:** альтернативная **IP-туннельная** сборка этого клиента живёт в бранче [`ip-tun`](../../tree/ip-tun). Вместо локального SOCKS-листенера она использует Android `VpnService` и поддерживает **per-app туннелирование** (туннель только для выбранных приложений). Только Android; скорость примерно как у 2G-мобильного подключения — для веб-сёрфинга не годится, но с Telegram работает на удивление сносно.
