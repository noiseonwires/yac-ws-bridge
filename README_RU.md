...
2. **Cloud Function** — задеплойте [`bridge-cloud/`](bridge-cloud/) в Yandex Cloud Functions и привяжите к API Gateway. В env-переменной `ADAPTER_URL` укажите HTTP-базу вашего адаптера (например `https://<сервер>:<порт>` — путь `/conn-ids` функция приклеивает сама), и используйте один и тот же `AUTH_TOKEN` (общий секрет) во всех трёх компонентах. Не забудьте обфусцировать JS перед заливкой (см. предупреждение выше).
...
**HTTP-путь `/conn-ids` — сейчас захардкожен в двух файлах.**

HTTP-эндпоинт адаптера, который Cloud Function опрашивает при холодном старте — `/conn-ids`. Чтобы переименовать его, надо поменять в двух местах:

- [`adapter-and-helper/cmd/adapter/main.go`](adapter-and-helper/cmd/adapter/main.go) — строка `mux.HandleFunc("/conn-ids", ...)`.
- [`bridge-cloud/index.js`](bridge-cloud/index.js) — строка, где собирается URL: `ADAPTER_URL.replace(/\/+$/, '') + '/conn-ids'`.

Дальше пересобрать адаптер и переразлить функцию. Строки в логах с `/conn-ids` можно оставить как есть — наружу они не видны.
