...
Два Go-бинарника — **adapter** (на стороне целевого сервера) и **helper** (на стороне клиента) — каждый держит upstream WebSocket-соединение к YC API Gateway. В режиме "без relay" данные передаются **напрямую** через YC WebSocket management API (gRPC `wsSend`), минуя Cloud Function на пути данных. Cloud Function нужна только чтобы клиент и сервер (хелпер и адаптер) нашли друг друга.
...
### 6. Просмотр логов Cloud Function
